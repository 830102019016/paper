...
The work in \cite{xiao2022energy} uses fractional programming and block coordinate descent to jointly optimize UAV trajectory and continuous communication resources. It relies on TDMA with unit bandwidth and does not consider spectrum-domain multiple-access or allocation strategies.
In \cite{wang2025joint}, a hierarchical framework is introduced to coordinate the movement of the UAVs with the choice of LEO satellites. The work focuses primarily on the structural design of the framework, and the optimization aimed at improving performance is not further developed.
...
In addition to the challenges in data collection, the delivery of the collected information to LEO satellites introduces another layer of complexity, as the UAV must decide how to offload data under temporally dynamic link quality, limited communication resources, and dynamic satellite visibility.
The study in \cite{wang2025coordinated} employs proximal policy optimization to determine the deployment of the UAV and the allocation of tasks to LEO satellites, focusing on reinforcement learning decisions for communication and computing resources.
The authors in \cite{zhang2023resource} consider the offloading of computational tasks from ground devices to LEO satellites with the aid of an UAV, and employ reinforcement learning for resource allocation.

While recent studies have examined data collection through unmanned aerial vehicles and task offloading toward low Earth orbit satellites, these efforts are often developed independently and primarily at a conceptual level. Although a hierarchical framework have been proposed, the underlying optimization problems have not been explored in depth. In particular, existing approaches rarely provide designs guided by system performance for collecting data with improved energy efficiency or for maintaining stable satellite association under dynamic visibility. These limitations motivate the development of a structured framework that improves both stages of the uplink process.

The main contributions of this work are summarized as follows:
\begin{itemize}

    \item A structured uplink framework is developed for data delivery in integrated space, air, and ground environments. The framework separates the process of collecting data from IoT devices and the process of delivering the collected information to LEO satellites, and it formalizes the optimization objectives of both stages in a consistent manner.
    ...
    \item A selection strategy driven by transmission demand is designed for the offloading phase to enhance the quality of uplink transmission under dynamic satellite visibility. The strategy maintains stable connectivity by triggering satellite switching only when required to satisfy the remaining transmission demand, thereby reducing interruptions and packet loss.
