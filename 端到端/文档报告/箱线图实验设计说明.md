...
展示4个对照组（G1, G2, G3, G4）在**端到端系统能耗**指标上的**数据分布特性**，包括：
...
| 组别 | 算法1 | 算法2 | 说明 |
|------|-------|-------|------|
| **G1** | P1优化 | Demand-Aware | 🌟 提出方法（完整优化） |
| **G2** | P1优化 | Greedy | 半优化A（仅算法1优化） |
| **G3** | F-scheme | Demand-Aware | 半优化B（仅算法2优化） |
| **G4** | F-scheme | Greedy | 传统基线（无优化） |
...
| 组别 | 种子27 | 种子37 | 种子41 | 总数据点 |
|------|--------|--------|--------|---------|
| G1 | 1个能耗值 | 1个能耗值 | 1个能耗值 | 3个 |
| G2 | 1个能耗值 | 1个能耗值 | 1个能耗值 | 3个 |
| G3 | 1个能耗值 | 1个能耗值 | 1个能耗值 | 3个 |
| G4 | 1个能耗值 | 1个能耗值 | 1个能耗值 | 3个 |

**注**: 3个数据点是箱线图的最小要求，可以计算四分位数。
...
1. **端到端指标**: 覆盖算法1和算法2两个阶段
2. **主要优化目标**: 算法1的核心优化目标
3. **可测量性**: 直接从实验结果中提取
4. **实用价值**: 能耗是IoT-UAV系统的关键性能指标
